...
export class Transaction {
  ...
  getRequirementsCheckResult(): RequirementsCheckResult | undefined {
    for (const argument of _get(this.googleAction.$request, 'inputs[0]["arguments"]', [])) {
  ...
  getTransactionDecisionResult(): TransactionDecision | undefined {
    for (const argument of _get(this.googleAction.$request, 'inputs[0]["arguments"]', [])) {
      if (argument.name === 'TRANSACTION_DECISION_VALUE') {
        return (
          _get(argument, 'extension.transactionDecision') ||
...
export class TransactionsPlugin implements Plugin {
  ...
  type(googleAction: GoogleAction) {
    if (
      _get(googleAction.$request, 'inputs[0].intent') ===
    ...
    if (
      _get(googleAction.$request, 'inputs[0].intent') === 'actions.intent.DIGITAL_PURCHASE_CHECK'
    ) {
      _set(googleAction.$type, 'type', 'ON_TRANSACTION');
      _set(googleAction.$type, 'subType', 'DIGITAL_PURCHASE_CHECK');
    }

    if (_get(googleAction.$request, 'inputs[0].intent') === 'actions.intent.DELIVERY_ADDRESS') {
      _set(googleAction.$type, 'type', 'ON_TRANSACTION');
      _set(googleAction.$type, 'subType', 'DELIVERY_ADDRESS');
    }
    //
    if (_get(googleAction.$request, 'inputs[0].intent') === 'actions.intent.COMPLETE_PURCHASE') {
      _set(googleAction.$type, 'type', 'ON_TRANSACTION');
      _set(googleAction.$type, 'subType', 'ON_COMPLETE_PURCHASE');
    }

    if (_get(googleAction.$request, 'inputs[0].intent') === 'actions.intent.TRANSACTION_DECISION') {
      if (
        _get(googleAction.$request, 'inputs[0].arguments[0].name') === 'TRANSACTION_DECISION_VALUE'
      ) {
        _set(googleAction.$type, 'type', 'ON_TRANSACTION');
